...
document.addEventListener('DOMContentLoaded', function(event) {
  ...
  const allCloses = document.querySelectorAll('[data-trigger]');
  for (const i = 0; i < allTriggers.length; i++) {
    allTriggers[i].addEventListener('click', function() {
      /*
        Might use the value for the data trigger later to specify different actions for different types of popup.
      */
      // const trigger = allTriggers[i].dataset.trigger;
      const target = Focus.getTarget(allTriggers[i], event);
    }
  }
